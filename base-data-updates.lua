...
if mods["space-age"] then
  ...
  if settings.startup["early-crushing-space-crusher"].value then
    data.raw["assembling-machine"]["crusher"].crafting_speed = 1.25
...
if compat.glass then
  data.raw.recipe["sand"].category = "basic-crushing-or-hand-crafting"
else
  if settings.startup["early-crushing-byproducts"].value then
  ...
  if settings.startup["early-crushing-glass"].value then
    ...
    frep.add_ingredient("lamp", {type="item", name="glass", amount=2})
...
if settings.startup["early-crushing-ore"].value then
  ...
  if mods["space-age"] then
    ...
    if settings.startup["early-crushing-byproducts"].value then
      ...
      frep.add_result("crushed-tungsten-ore", {type="item", name=settings.startup["early-crushing-coal"].value and "crushed-coal" or "coal", amount=1, probability=0.02, show_details_in_recipe_tooltip=false})
      if settings.startup["early-crushing-big-crusher"].value then
...
if settings.startup["early-crushing-coal"].value then
  frep.replace_ingredient("coal-liquefaction", "coal", "crushed-coal")
  frep.replace_ingredient("explosives", "coal", "crushed-coal")
  frep.replace_ingredient("plastic-bar", "coal", "crushed-coal")
  frep.replace_ingredient("carbon", "coal", "crushed-coal")
  frep.replace_ingredient("poison-capsule", "coal", "crushed-coal")
  frep.replace_ingredient("slowdown-capsule", "coal", "crushed-coal")
...
if mods["space-age"] then
  if mods["Rocky-Asteroids"] and not settings.startup["early-crushing-byproducts"].value then
    data.raw["furnace"]["electric-crusher"].result_inventory_size = 3
  end

  -- byproducts from asteroid crushing
  if settings.startup["early-crushing-byproducts"].value and not mods["Rocky-Asteroids"] then
    ...
    if settings.startup["early-crushing-ore"].value and not mods["cupric-astroids"] then
    ...
    frep.add_result("advanced-carbonic-asteroid-crushing", {type="item", name=settings.startup["early-crushing-coal"].value and "crushed-coal" or "coal", amount=5, probability=0.15, show_details_in_recipe_tooltip=false})
    frep.add_result("advanced-carbonic-asteroid-crushing", {type="item", name=settings.startup["early-crushing-coal"].value and "coal" or "stone", amount=2, probability=0.15, show_details_in_recipe_tooltip=false})
  ...
  if compat.glass or settings.startup["early-crushing-glass"].value then
    if settings.startup["early-crushing-byproducts"].value or mods["Rocky-Asteroids"] then
